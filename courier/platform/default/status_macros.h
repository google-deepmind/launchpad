...
#ifndef COURIER_PLATFORM_DEFAULT_STATUS_H_
#define COURIER_PLATFORM_DEFAULT_STATUS_H_

#include "absl/status/status.h"
#include "courier/platform/default/status_builder.h"
...
//     COURIER_RETURN_IF_ERROR(Function(args...));
//     COURIER_RETURN_IF_ERROR(foo.Method(args...));
...
// For example:
//   absl::Status MultiStepFunction() {
//     COURIER_RETURN_IF_ERROR(Function(args...)) << "in MultiStepFunction";
//     COURIER_RETURN_IF_ERROR(foo.Method(args...)).Log(base_logging::ERROR)
...
//   }
//
//   COURIER_RETURN_IF_ERROR(foo()).With(TeamPolicy);
//   COURIER_RETURN_IF_ERROR(bar()).With(TeamPolicy);
//
...
//     return absl::OkStatus();
//   }
#define COURIER_RETURN_IF_ERROR(expr)                                     \
...
// Interface:
//
//   COURIER_ASSIGN_OR_RETURN(lhs, rexpr)
//   COURIER_ASSIGN_OR_RETURN(lhs, rexpr, error_expression);
//
...
//   COURIER_ASSIGN_OR_RETURN(ValueType value, MaybeGetValue(arg));
//
// Example: Assigning to an existing variable:
//   ValueType value;
//   COURIER_ASSIGN_OR_RETURN(value, MaybeGetValue(arg));
//
// Example: Assigning to an expression with side effects:
//   MyProto data;
//   COURIER_ASSIGN_OR_RETURN(*data.mutable_str(), MaybeGetValue(arg));
//   // No field "str" is added on error.
//
// Example: Assigning to a std::unique_ptr.
//   COURIER_ASSIGN_OR_RETURN(std::unique_ptr<T> ptr, MaybeGetPtr(arg));
//
...
// to COURIER_RETURN_IF_ERROR. The expression may, however, evaluate to any type
// returnable by the function, including (void). For example:
//
// Example: Adjusting the error message.
//   COURIER_ASSIGN_OR_RETURN(ValueType value, MaybeGetValue(query),
...
//
#define COURIER_ASSIGN_OR_RETURN(...)                                        \
...
//
//   if (do_expr) COURIER_RETURN_IF_ERROR(expr) << "Some message";
//
...
class StatusAdaptorForMacros {
 public:
  ...
  StatusAdaptorForMacros(bool status, const char* file, int line)
      : builder_(status ? absl::OkStatus() : absl::InternalError(""), file,
                 line) {}
...
#define COURIER_CHECK_OK(val) CHECK_EQ(::absl::OkStatus(), (val))

#define COURIER_RET_CHECK(expr) COURIER_RETURN_IF_ERROR(ABSL_PREDICT_TRUE(expr))
#define COURIER_RET_CHECK_NE(a, b) COURIER_RET_CHECK(a != b);
#define COURIER_RET_CHECK_EQ(a, b) COURIER_RET_CHECK(a == b);

#endif  // COURIER_PLATFORM_DEFAULT_STATUS_H_
